...
void Shape3D::buildMenus() {
    ...
    // SAVE
    QAction* saveSTL = new QAction("Save STL", this);
    connect(saveSTL, &QAction::triggered, this, [this, stl]() {
        auto data = m_canvas->getMeshData();
        if (!data.empty())
            stl->saveSTL(data);
        });
    optMenu->addAction(saveSTL);

    // LOAD
    QAction* loadSTL = new QAction("Load STL", this);
    connect(loadSTL, &QAction::triggered, this, [this, stl]() {
        auto data = stl->loadSTL();
        if (!data.empty())
            m_canvas->loadMeshData(data);
        });
    optMenu->addAction(loadSTL);
}