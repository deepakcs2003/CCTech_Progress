...
void Circle::setEnd(QPoint p) {
    ...
    int r = std::min(std::abs(p.x() - m_start.x()),
        std::abs(p.y() - m_start.y())) / 2;
...
std::vector<float> Circle::getVertices() const {
    ...
    for (int i = 0; i < SEG; i++) {
        float t1 = 2 * PI * i / SEG;
        float t2 = 2 * PI * (i + 1) / SEG;

        v.push_back(cx + r * cosf(t1));
        v.push_back(cy + r * sinf(t1));

        v.push_back(cx + r * cosf(t2));
        v.push_back(cy + r * sinf(t2));
    }
...
// 3D
std::vector<float> Circle::getMesh3D() const
{
    float cx = pts[0].x();
    float cy = pts[0].y();
    float r = pts[1].x() - pts[0].x();
